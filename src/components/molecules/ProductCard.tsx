  ...
  return (
    <div className={cn("group relative flex flex-col gap-1", className)}>
      ...
      <div className="flex flex-col gap-0.5">
        <Link href={`/product/${product.id}`} className="text-[13px] font-semibold text-[#222222] leading-tight hover:underline line-clamp-2">
          {product.name}
        </Link>
        <span className="text-xs text-[#717171]">{product.vendorName} • {product.origin}</span>
      ...
      <Button size="sm" variant="outline" className="h-7 text-xs px-2" onClick={handleAddToCart} leftIcon={<HiShoppingCart className="h-3 w-3" />}>
        Cart
      </Button>
    </div>
  );
